...
fn (mut s Screen) erase_pinned() {
	...
	// the cursor sits on the LAST row of the pinned region after a draw, so
	// climbing rows-1 lands on its first row
	mut out := carriage_return
	if s.pinned_rows > 1 {
		out += cursor_up(s.pinned_rows - 1)
	}
	out += erase_to_end
	s.term.write(out)
	s.pinned_rows = 0
...
fn (mut s Screen) draw_pinned(lines [][]Span, cursor_row int, cursor_col int) {
	...
	// set the column absolutely
	rows_from_bottom := s.pinned_rows - 1 - cursor_row
